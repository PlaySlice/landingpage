...
const AIWebsiteSection = ({ isWeb3 = false }) => {
  ...
  const opacity = useTransform(scrollYProgress, [0, 0.3, 0.6, 1], [0.4, 1, 1, 0.4]);
  ...
  const FloatingParticle = ({ 
    ...
    <motion.div
      ...
      className={`absolute rounded-full ${isWeb3 ? 'bg-white' : 'bg-indigo-500'}`}
      style={{
        width: size,
        height: size,
        bottom: '10%',
  ...
  return (
    <section
      id="ai-website"
      ref={sectionRef}
      className={`py-32 relative overflow-hidden ${isWeb3 ? "bg-gradient-to-b from-[#FF5C3A]/10 via-[#FFB07C]/10 to-[#FF6A4D]/10" : "bg-neutral-50 dark:bg-neutral-900"}`}
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-b opacity-30 pointer-events-none" style={{
        background: isWeb3 
      ...
      <motion.div style={{ opacity }} className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-20">
          ...
          >
            <div className={`px-4 py-1.5 rounded-full text-sm font-medium mb-6 inline-flex ${isWeb3 ? "bg-white/10 text-[#18181b] backdrop-blur-md border border-[#18181b]/20" : "bg-neutral-900/5 text-neutral-600 dark:bg-white/5 dark:text-neutral-400"}`}>
              <span className="relative inline-flex items-center">
                AI-Driven Experience
                <span className={`ml-2 w-2 h-2 rounded-full ${isWeb3 ? 'bg-[#FF5C3A]' : 'bg-indigo-500'} animate-ping absolute -right-4`}></span>
                <span className={`ml-2 w-2 h-2 rounded-full ${isWeb3 ? 'bg-[#FF5C3A]' : 'bg-indigo-500'} absolute -right-4`}></span>
              </span>
            </div>
          </motion.div>
          ...
          <motion.h2
            ...
            className={`text-4xl md:text-5xl font-bold mb-6 tracking-tight ${isWeb3 ? "text-[#18181b] drop-shadow-lg" : "text-neutral-900 dark:text-white"}`}
          ...
          <motion.p
            ...
            className={`text-lg max-w-xl mx-auto ${isWeb3 ? "text-[#18181b]/80" : "text-neutral-600 dark:text-neutral-400"}`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            >
              ...
              <div className={`absolute inset-0 backdrop-blur-[2px] opacity-20 ${isWeb3 ? 'bg-gradient-to-br from-[#FF5C3A]/20 to-[#FFB07C]/10' : 'bg-gradient-to-br from-blue-500/20 to-indigo-500/10'}`}></div>
              ...
              >
                <Zap className={`w-5 h-5 ${isWeb3 ? "text-[#FF5C3A]" : "text-indigo-500"}`} />
              </motion.div>
            </motion.div>
            
            {/* Decorative elements */}
            <div className={`absolute -z-10 -bottom-6 -left-6 w-64 h-64 rounded-full opacity-20 ${isWeb3 ? 'bg-[#FF5C3A]' : 'bg-indigo-500'} blur-3xl`}></div>
            <div className={`absolute -z-10 -top-6 -right-6 w-48 h-48 rounded-full opacity-10 ${isWeb3 ? 'bg-[#FF5C3A]' : 'bg-indigo-500'} blur-2xl`}></div>
          </motion.div>
          ...
          >
            <div className="space-y-8">
              ...
              {tabs.map((tab) => (
                ...
                >
                  <h3 className={`text-2xl font-bold mb-3 ${isWeb3 ? "text-[#18181b]" : "text-neutral-900 dark:text-white"}`}>
                    {tab.label} your website with AI
                  </h3>
                  <p className={`mb-6 ${isWeb3 ? "text-[#18181b]/80" : "text-neutral-600 dark:text-neutral-400"}`}>
                  ...
                  <ul className="space-y-3 mb-8">
                    ...
                    >
                      <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mr-3 mt-1 ${isWeb3 ? 'bg-[#FF5C3A]' : 'bg-green-500'}`}>
                        <svg
                      ...
                      <span className={`${isWeb3 ? "text-[#18181b]" : "text-neutral-700 dark:text-neutral-300"}`}>
                        {tab.id === "design" &&
